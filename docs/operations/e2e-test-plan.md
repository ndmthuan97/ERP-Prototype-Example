...
> Bộ E2E test coverage toàn bộ 9 business flows chính, chạy qua API Gateway (`localhost:3010`) trên local dev environment (`npm run dev:all`, DB = Cloud SQL).
...
- **Test chạy trên data thật (Cloud SQL PostgreSQL).** E2E test sẽ tạo/sửa/xóa data thực trên DB.
- Dùng **cleanup per test** — tạo data đầu suite, dọn cuối suite.
- Đây là learning project nên chạy trực tiếp trên schema chính.

### Rate Limiting

- API Gateway có rate limit **100 req/15 min** global và **5/15 min** cho `/auth/sso/callback`.
...
│   ├── 02-auth.e2e.ts      [NEW] — SSO callback (Google sign-in), me, logout
...
- Provision admin trong allowlist (`app_auth.users`) → **Sign in với Google** (Firebase emulator / test ID token) → `POST /auth/sso/callback` → get app token HS256
...
| # | Test Case | Expected |
|---|-----------|----------|
| 1 | SSO callback với Firebase ID token hợp lệ (admin) | 200 + accessToken (app token HS256) + user |
| 2 | SSO callback email chưa provision trong allowlist | 403 |
| 3 | SSO callback ID token không hợp lệ / hết hạn | 401 |
| 4 | Access protected route without token | 401 |
| 5 | Access protected route with valid app token | 200 |
| 6 | GET /auth/me | 200 + user info |
| 7 | Logout (revoke session) | 204 |
| 8 | Dùng lại app token sau logout | 401 (session bị xoá khỏi whitelist) |

**Pattern tested:** Identity Platform SSO, app token HS256, session whitelist (revoke tức thì), RBAC.

> ⚙️ **CI note:** popup "Sign in with Google" thật không chạy headless được — dùng **Firebase Auth emulator** hoặc **test ID token** (mint qua firebase-admin) để lấy ID token đưa vào `/auth/sso/callback`.
...
```mermaid
graph TD
    A["01-health<br>(smoke test)"] --> B["02-auth<br>(SSO → app token)"]
