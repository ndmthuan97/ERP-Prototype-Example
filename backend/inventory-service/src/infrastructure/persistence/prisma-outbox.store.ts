// =============================================================================
// PRISMA OUTBOX STORE — Adapter cho OutboxStore (port @erp/shared) — Inventory
// =============================================================================
// Giống customer-service: CLAIM bằng FOR UPDATE SKIP LOCKED (an toàn đa-instance)
// + retry/DLQ. Khác đúng tên schema "inventory".
...
export class PrismaOutboxStore implements OutboxStore {
  ...
  async fetchUnpublished(limit: number): Promise<OutboxRecord[]> {
    const rows = await this.prisma.$queryRaw<ClaimedRow[]>`
      ...
      WHERE "id" IN (
        SELECT "id" FROM "inventory"."outbox"
