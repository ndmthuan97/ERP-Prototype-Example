...
// =============================================================================
// OutboxWorkerService (generic, in @erp/shared) does NOT know Prisma.
// This adapter connects the generic worker to the purchasing-service outbox table.
//
...
export class PrismaOutboxStore implements OutboxStore {
  ...
  /**
   * CLAIM up to `limit` unpublished events (FIFO by created_at).
  ...
  async fetchUnpublished(limit: number): Promise<OutboxRecord[]> {
    const rows = await this.prisma.$queryRaw<ClaimedRow[]>`
      UPDATE "purchasing"."outbox"
      SET "locked_until" = now() + (${LOCK_SECONDS} * interval '1 second')
      WHERE "id" IN (
        SELECT "id" FROM "purchasing"."outbox"
