...
describe('SearchCustomersQuery', () => {
  it('dùng default page=1, limit=20 khi không truyền', async () => {
    ...
    await query.execute();
    expect(repo.search).toHaveBeenCalledWith('', 1, 20, undefined);
  });
  ...
  it('clamp limit > 100 về 100 và page < 1 về 1', async () => {
    ...
    expect(repo.search).toHaveBeenCalledWith('acme', 1, 100, undefined);
  });
  ...
  it('trim query trước khi delegate', async () => {
    ...
    expect(repo.search).toHaveBeenCalledWith('acme', 2, 50, undefined);
  });

  it('forward status filter xuống repository khi được cung cấp', async () => {
    const repo = makeRepoMock();
    const query = new SearchCustomersQuery(repo);
    await query.execute('acme', 1, 20, 'active');
    expect(repo.search).toHaveBeenCalledWith('acme', 1, 20, 'active');
  });
});
